...
import FlightItineraryCard, { Itinerary } from '../components/FlightItineraryCard';
      ...
      <div className="w-full max-w-5xl">
        ...
        {results && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h2 className="text-2xl font-bold mb-6 text-white px-2">Search Results</h2>
            {results.length === 0 ? (
              <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl shadow-xl border border-dashed border-slate-600 p-12 text-center flex flex-col items-center justify-center">
                <svg className="w-16 h-16 text-slate-500 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <h3 className="text-xl font-semibold text-slate-200 mb-2">No Flights Found</h3>
                <p className="text-slate-400 max-w-md mx-auto">We couldn't find any itineraries matching your criteria. Try adjusting your dates or searching for different airports.</p>
              </div>
            ) : (
              <div className="space-y-6">
                {results.map((itinerary: Itinerary, idx: number) => (
                  <FlightItineraryCard key={idx} itinerary={itinerary} />
                ))}
              </div>
            )}
          </div>
